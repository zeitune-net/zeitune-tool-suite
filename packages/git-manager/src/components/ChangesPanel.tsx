  ...
  return (
    <div className="group flex items-center gap-2 rounded-lg px-2 py-1 hover:bg-muted/50">
      ...
      <button
        ...
        onClick={() => onViewDiff(path, type === 'staged')}
      >
        {fileName}
      </button>
...
export function ChangesPanel({ repo }: { repo: Repository }) {
  const { stageFiles, unstageFiles, stageAllFiles, discardFiles } = useGitManagerStore()
